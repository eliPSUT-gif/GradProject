...
export function MessagingProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`messaging:${currentAppUserId}`, {
      config: {
        presence: {
    ...
    channel
      ...
      .subscribe((status) => {
        ...
        void channel.track({ universityId: user.id, connectedAt: new Date().toISOString() });
        void touchLastSeen();
      });
  ...
  const getPresence = useCallback(
    (userId: string) => {
      ...
      return {
        isOnline: userId === user?.id || Boolean(appUserId && onlineAppUserIds.has(appUserId)),
